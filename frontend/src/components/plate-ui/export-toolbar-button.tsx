...
import { TPlateEditor, useEditorRef } from "@udecode/plate/react";
...
export function ExportToolbarButton({ children, ...props }: DropdownMenuProps) {
	const editor: TPlateEditor = useEditorRef();
	...
	const exportToDocx = async () => {
		const editorhtmlString = await editorToHtml();
		const parser = new DOMParser();
		const htmlContent = parser.parseFromString(
			editorhtmlString,
			"text/html"
		).body;
		// const htmlContent = editor.api.toDOMNode(editor);

		if (htmlContent) {
			const doc = parseHtmlToDocx(htmlContent);
			await Packer.toBlob(doc).then((blob: any) => {
				const url = URL.createObjectURL(blob);
				downloadFile({ content: url, filename: "plate.docx" });
			});
		}
	};
	...
	const exportToHtml = async () => {
		const editorHtml = await editorToHtml();
...
function parseHtmlToDocx(htmlContent: HTMLElement) {
	const sections: any[] = [];
	for (let el of htmlContent.children) {
		const paragraph = formatNode(el as HTMLElement);
		if (paragraph) {
		...
		switch (node.tagName) {
			...
			case "H3":
				return new Paragraph({
					...
					indent: { left: parseInt(node.style.textIndent, 10) || 0 },
				});
			case "P":
				return new Paragraph({
